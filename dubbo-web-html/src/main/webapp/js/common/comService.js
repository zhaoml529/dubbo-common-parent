 ...
 * $http.patch
 */
app.service('comService', ['$http', '$q', 'host',function($http, $q, host) {
	return {
		// 分页列表
	    listPage:function(params,address) {
	    ...
	    // 添加用户
	    addUser:function(params,address) {
	    	var deferred = $q.defer();
	    	$http({
                method: "post",
                data: angular.toJson(params),//JsonData = {"id":1,"value":"hello"}
	    ...
	    // 编辑用户
	    editUser:function(params,address) {
	    	var deferred = $q.defer();
	    	$http({
                method: "put",
                data: angular.toJson(params),//JsonData = {"id":1,"value":"hello"}
                url: host+address+user.id,
                headers: { "Content-Type": "application/json" }
            }).success(function (d) { 
            	deferred.resolve(d);
            });
		...
		//锁定用户
	    lockUser:function(id, status,address,address2) {
	    	var deferred = $q.defer();
	    	$http({
                method: "get",
                url: host+address+id+address2+status,
            }).success(function (d) { 
            	deferred.resolve(d);
            });
	    	return deferred.promise;
